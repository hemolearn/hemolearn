...
from seven.utils import sort_atoms_by_explained_variances
...
fmri_img, anat_img = get_hcp_fmri_fname(subject_id, anat_data=True)
masker = input_data.NiftiMasker(t_r=TR_HCP,
                                smoothing_fwhm=6,
                                detrend=True,
                                standardize=True,
                                low_pass=0.1,
...
cdl = SLRDM(n_atoms=n_atoms, v=v, lbda=1.0e-1, max_iter=50,
...
u_hat, Lz_hat = cdl.u_hat, cdl.Lz_hat
u_hat, Lz_hat, variances = sort_atoms_by_explained_variances(u_hat, Lz_hat, v)

###############################################################################
# results management
date = datetime.now()
dirname = 'results_hcp_#{0}{1}{2}{3}{4}{5}'.format(date.year, date.month,
                                                   date.day, date.hour,
